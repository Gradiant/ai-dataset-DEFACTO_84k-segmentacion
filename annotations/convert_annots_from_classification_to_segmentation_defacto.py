    ...
    for image in images_obj:
        ...
        if annotation["category_id"] == 0:
            ...
            mask_file = os.path.join(
                manipulated_masks_folder, file_name[2:]
            )
            logger.info(f"Input filename {file_name}")
            logger.info(f"Mask filename {mask_file}")

            if not os.path.isfile(mask_file):
                tiff_mask_file = os.path.join(
                    manipulated_masks_folder, filename[2:]+'.tif'
                )
                logger.info(f"Trying to load tif image {tiff_mask_file}")
                if not os.path.isfile(tiff_mask_file):
                    logger.info(f"Image {tiff_mask_file} does not exist")
                mask_tif = cv2.imread(tiff_mask_file)
                cv2.imwrite(mask_file, mask_tif)
            # load the image
